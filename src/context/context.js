...
const GithubProvider = ({children}) => {
    ...
    const searchGithubUser = async (user) => {
        ...
        if (response){
            ...
            //repos
            axios(`${repos_url}?per_page=100`).then((response) => setRepos(response.data)).catch((err) => console.log(err))
            // followers
            axios(`${followers_url}?per_page=100`).then((response) => setFollowers(response.data)).catch((err) => console.log(err))
